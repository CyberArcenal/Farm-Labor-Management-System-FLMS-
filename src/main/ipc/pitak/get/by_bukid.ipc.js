...
// @ts-ignore
module.exports = async (/** @type {any} */ bukidId, filters = {}, /** @type {any} */ userId) => {
  try {
    if (!bukidId) {
    ...
    const pitaksWithStats = await Promise.all(pitaks.map(async (/** @type {{ id: any; location: any; totalLuwang: string; status: any; createdAt: any; updatedAt: any; }} */ pitak) => {
      ...
      return {
        ...
        utilizationRate: parseFloat(pitak.totalLuwang) > 0 
          ? (parseFloat(assignmentStats.totalLuWangAssigned || 0) / parseFloat(pitak.totalLuwang)) * 100 
          : 0,
    ...
    const bukidStats = pitaksWithStats.reduce((/** @type {{ totalPitaks: number; totalLuWangCapacity: any; totalLuWangAssigned: any; totalAssignments: any; totalActiveAssignments: any; activePitaks: number; inactivePitaks: number; harvestedPitaks: number; }} */ stats, /** @type {{ totalLuwang: any; assignmentStats: { totalLuWangAssigned: any; total: any; active: any; }; status: string; }} */ pitak) => {
      stats.totalPitaks++;
      ...
      if (pitak.status === 'harvested') stats.harvestedPitaks++;
    ...
    bukidStats.utilizationRate = bukidStats.totalLuWangCapacity > 0 
      ? (bukidStats.totalLuWangAssigned / bukidStats.totalLuWangCapacity) * 100 
      : 0;
