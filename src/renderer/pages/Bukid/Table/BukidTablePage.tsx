...
const BukidTablePage: React.FC = () => {
  ...
  // 4. Pwedeng i-refresh ang data pagkatapos mag-add ng plot:
  const handleAddPlotSuccess = async () => {
    await fetchBukids(); // Refresh the bukid list
    closeAddPlotDialog();
  ...
  // 2. Palitan ang placeholder na `openViewPlotsDialog` function:
  ...
  const openViewNoteDialog = (id: number) => {
    ...
    if (bukid && bukid.notes) {
      setSelectedNote({
        ...
        note: bukid.notes,
        createdAt: bukid.createdAt || new Date().toISOString(),
        updatedAt: bukid.updatedAt || new Date().toISOString(),
      });
  ...
  // Sa mga ibang functions, idagdag ang delete note handler:
  ...
  const handleViewMap = async (id: number) => {
    await dialogs.alert({
      title: "View Map",
          ...
          onClose={async () => {
            if (
              !(await dialogs.confirm({
                title: "Close Bukid View",
                message: "Are you sure you want to close the bukid view?",
              }))
            )
              return;
            closeFormDialog();
      ...
      {isViewPlotsDialogOpen && selectedBukidId && (
        <ViewPlotsDialog
          ...
          onViewPlotDetails={(plotId) => {
            // Maaari kang magdagdag ng function para tingnan ang plot details
            setSelectedViewPitakId(plotId);
            setShowViewPitakDialog(true);
            closeViewPlotsDialog();
            console.log("View plot details:", plotId);
          }}
        />
      )}
